...
const rotateLink = document.createElement('button')
	...
	rotateLink
		.addEventListener('click', (event) => {
			if (event.target.classList.contains('button--not-in-use')) {
				return;
			}
...
const drawImage = ({image}) => {
	...
	if (rotation === 270) {
		ctx.translate(0, rotationOffset);
	}
...
reader
	.addEventListener('load', () => {
		...
		document.querySelectorAll('.button--not-in-use').forEach( (obj) => obj.classList.remove('button--not-in-use'))
